...
layui.use('laytpl', async function () {
    ...
    const modal0 = `<div class="layui-row">
                      <a href="/pages/findTeam/findTeam.html?field=industry&value=all" class="layui-col-md2" style="text-align: center;color: #01AAED;">全部</a>
                      {{#  layui.each(d.list, function(index, item){ }}
                      <a href="/pages/findTeam/findTeam.html?field=industry&value={{ item.industry }}" class="layui-col-md2" style="text-align: center;color: #01AAED;">{{ item.industry }}</a>
                      {{# }); }}
                  </div>`;

    var industryList = await fetch(socket + '/getDistinct?table=team&field=industry')
        .then(res => res.json())
        .then(res => res);
    ...
    const modal1 = `<div class="layui-row">
                      <a href="/pages/findTeam/findTeam.html?field=school&value=all" class="layui-col-md2" style="text-align: center;color: #01AAED;">全部</a>
                      {{#  layui.each(d.list, function(index, item){ }}
                      <a href="/pages/findTeam/findTeam.html?field=school&value={{ item.school }}" class="layui-col-md2" style="text-align: center;color: #01AAED;">{{ item.school }}</a>
                      {{# }); }}
                  </div>`;

    var schoolList = await fetch(socket + '/getDistinct?table=team&field=school')
        .then(res => res.json())
        .then(res => res);
...
layui.use(['laypage', 'laytpl'],async function () {
  ...
  {{#  layui.each(d.list, function(index, item){ }}
  <a href="/pages/detail/teamDetail.html?type=team&id={{ item.id }}">
    <div class="layui-row">
      ...
      <div class="layui-col-md4" style="word-break: keep-all;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
        {{item.introduction}}
      </div>
    </div>
  </a>
  {{# }); }}`;
    ...
    var teamList = await fetch(socket + `/getTeamInfo?field=${params['field']}&value=${params['value']}`)
      .then(res => res.json())
      .then(res => res);
    ...
    laypage.render({
      ...
      , count: teamList.length //数据总数，从服务端得到
      ,jump: function(obj, first){
        ...
        var data = { //数据
          list: teamList.slice((obj.curr - 1)*obj.limit, obj.curr*obj.limit + obj.limit)
        }
